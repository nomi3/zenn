...
自然性は $\mathrm{ApiResult}$ と $\mathrm{Array}$ の `fmap` が $\sigma$ を通して整合すること、つまり次の四角形が可換であることです。
...
`ApiResult<A>` は2通りしかないので、両方試せば確認できます。
...
自然性を確かめるには、$\tau$ の四角形と $\sigma$ の四角形を縦に積みます。
...
上の四角形は $\tau$ の自然性、下の四角形は $\sigma$ の自然性で、どちらも可換です。このとき外側の長方形 ($\mathrm{Optional}$ から $\mathrm{Array}$ まで) も可換になります。それが $\sigma \circ \tau$ の自然性です。

式で追うと、$\tau$ と $\sigma$ の自然性を1回ずつ使うだけです。
...
2行目で $\tau$ の自然性、3行目で $\sigma$ の自然性を使っています。$\mathrm{Optional}$ や $\mathrm{ApiResult}$ の中身には一度も触れていないので、この議論はどんな関手・自然変換でも通ります。**自然変換どうしを合成すると自然変換になる**、が一般に言えます。
...
結合律も確認が要りますが、自然変換の合成は各成分での関数合成なので、関数合成が結合的であることからそのまま従います。単位律も同様です。

圏 $\mathcal{C}$ から圏 $\mathcal{D}$ への関手全体は、こうして圏になります。$\mathcal{D}^{\mathcal{C}}$ や $\mathrm{Fun}(\mathcal{C}, \mathcal{D})$ と書かれます。

関手と自然変換を別々に見ていたときは、関手が「型を作るもの」、自然変換が「型を変換するもの」という別種の道具に見えていました。関手圏として見ると、関手は点で自然変換は矢印という、いつもの圏の景色になります。

# まとめ

- 関手圏は、関手を対象・自然変換を射とする圏
- 自然変換の合成は各成分で関数を合成するだけ。合成した結果もまた自然変換になる
- その証明は、可換な四角形を縦に積んで外側の長方形も可換になることを見るだけで、関手の中身には触れない
